                ...
                <div className="flex flex-wrap justify-between items-center mx-auto max-w-screen-xl">
                    ...
                    <div className="hidden justify-between items-center w-full lg:flex lg:w-auto lg:order-1" id="mobile-menu-2">
                        <ul className="flex flex-col mt-4 font-medium lg:flex-row lg:space-x-8 lg:mt-0">
                            {homeNavLinks && homeNavLinks.map(navLink => (
                                <li key={navLink.name}>
                                    <Link href={navLink.link} className={`block py-2 pr-4 pl-3 ${path === navLink.link ? "text-white" : "text-gray-400"} rounded lg:bg-transparent lg:p-0`} aria-current="page">{navLink.name}</Link>
                                </li>
                            ))}
                        </ul>
                    </div>
