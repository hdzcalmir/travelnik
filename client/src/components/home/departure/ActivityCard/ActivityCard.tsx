...
const ActivityCard = ({ activity }: ActivityCardProps) => {
    ...
    return (
        <tr className="border-b dark:border-gray-700 hover:bg-gray-700 transition duration-500 cursor-pointer">
            <th scope="row" className="px-4 py-3 font-medium text-gray-900 whitespace-nowrap dark:text-white">{activity.name}</th>
            ...
            <td className="px-4 py-3">
                <div className="flex items-center">
                    ...
                    <p className="text-gray-400 font-bold text-sm ml-1">{averageRate}<span className="text-gray-400 font-normal">({activity.reviews?.length > 0 ? activity.reviews?.length : 0} reviews)</span>
                    </p>
                </div>
            </td>
