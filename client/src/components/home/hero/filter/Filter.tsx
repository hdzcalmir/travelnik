...
const Filter = ({
  ...
  const fetchDataForGivenFilters = async () => {
    ...
    if (
      departureTime.check_in.length === 0 ||
      departureTime.check_out.length === 0
    ) {
    ...
    router.push(
      `/departure?interests=${btoa(
        JSON.stringify(selectedInterests)
      )}&check_in=${departureTime.check_in}&check_out=${
        departureTime.check_out
      }&people=${peopleComing}`
    );
  };
  ...
  return (
    <div className="grid gap-y-4 p-4 mt-8 w-full bg-white rounded lg:gap-x-4 lg:grid-cols-9 lg:mt-12 dark:bg-gray-800">
      <div className="lg:col-span-3">
        <label htmlFor="interest-form" className="sr-only">
          Interests
        </label>
        <div className="relative">
          ...
          <div ref={interestRef}>
            ...
            {interestDropdown && (
              <div
                id="dropdownSearch"
                className="z-10 absolute bg-white top-10 shadow w-60 dark:bg-gray-700 rounded-b-lg"
              >
                <ul
                  className="h-48 px-3 scrollbar-hidden pb-3 overflow-y-auto text-sm text-gray-700 dark:text-gray-200"
                    ...
                    interests.map((interest) => (
                      <li key={interest}>
                        <div className="flex items-center pl-2 rounded hover:bg-gray-100 dark:hover:bg-gray-600">
                          ...
                          />
                          <label
                            htmlFor={`checkbox-item-${t(interest)}`}
      ...
      <div className="lg:col-span-1">
        ...
        <select
          ...
          onChange={(e: ChangeEvent<HTMLSelectElement>) =>
            handlePeopleComing(e)
          }
          value={peopleComing}
          className="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-secondaryColor focus:border-secondaryColor block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-secondaryColor dark:focus:border-gray-200"
        >
      ...
      <button
        type="submit"
        onClick={fetchDataForGivenFilters}
